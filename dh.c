...
DH* DHInit(int nKeyBits)
{
	...
	if(!dh)
        ...
        p = BN_new();
        res = BN_hex2bn(&p, P1024); // prime P1024, see dhgroups.h
        if(!res) { goto failed; }

        g = BN_new();
        res = BN_set_word(g, 2);    // base 2
        if(!res) { goto failed; }

        DH_set0_pqg(dh, p, BN_new(), g);

	if(!DH_get0_p(dh) || !DH_get0_g(dh))
		goto failed;
...
int DHGenerateKey(DH *dh)
{
	...
	while(!res)
	{
		...
		assert(BN_hex2bn(&q1, Q1024));
                ...
                pub_key = BN_copy(pub_key, DH_get0_pub_key(dh));
                
                BIGNUM *p = BN_new();
                p = BN_copy(p, DH_get0_p(dh));
...
int DHGetPublicKey(DH *dh, uint8_t *pubkey, size_t nPubkeyLen)
{
  ...
  int len = BN_num_bytes(DH_get0_pub_key(dh));
  if(len <= 0 || len > (int)nPubkeyLen)
    return 0;

  memset(pubkey, 0, nPubkeyLen);
  BN_bn2bin(DH_get0_pub_key(dh), pubkey + (nPubkeyLen - len));
  return 1;
}

int DHGetPrivateKey(DH *dh, uint8_t *privkey, size_t nPrivkeyLen)
{
  if(!dh || !DH_get0_priv_key(dh))
    return 0;
        
  int len = BN_num_bytes(DH_get0_priv_key(dh));
  if(len <= 0 || len > (int)nPrivkeyLen)
    return 0;
  
  memset(privkey, 0, nPrivkeyLen);
  BN_bn2bin(DH_get0_priv_key(dh), privkey + (nPrivkeyLen - len));
  return 1;
}
